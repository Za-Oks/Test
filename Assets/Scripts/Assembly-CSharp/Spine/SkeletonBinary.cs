	...
	public class SkeletonBinary
	{
		...
		private void ReadAnimation(string name, Stream input, SkeletonData skeletonData)
		{
			...
			for (int num18 = ReadVarint(input, true); num17 < num18; num17++)
			{
				...
				IkConstraintTimeline ikConstraintTimeline2 = ikConstraintTimeline;
				for (int num20 = 0; num20 < num19; num20++)
				{
					ikConstraintTimeline2.SetFrame(num20, ReadFloat(input), ReadFloat(input), ReadSByte(input), ReadBoolean(input), ReadBoolean(input));
					if (num20 < num19 - 1)
					{
						ReadCurve(input, num20, ikConstraintTimeline2);
					}
				}
				exposedList.Add(ikConstraintTimeline2);
				num = Math.Max(num, ikConstraintTimeline2.frames[(num19 - 1) * 5]);
			}
			int num21 = 0;
			...
			if (num59 > 0)
			{
				...
				for (int num60 = 0; num60 < num59; num60++)
				{
					...
					{
						event2.volume = ReadFloat(input);
						event2.balance = ReadFloat(input);
					}
					eventTimeline.SetFrame(num60, event2);
				}
