...
namespace Spine
{
	public class PathConstraint : IConstraint, IUpdatable
	{
		...
		public void Update()
		{
			...
			if (flag5 || !flag3)
			{
				...
				while (num5 < num6)
				{
					...
					else if (flag3)
					{
						...
						exposedList.Items[++num5] = num4;
					}
					else
					{
						float num10 = length * bone.a;
						float num11 = length * bone.c;
						float num12 = (float)Math.Sqrt(num10 * num10 + num11 * num11);
						if (flag5)
						{
							exposedList2.Items[num5] = num12;
						}
						exposedList.Items[++num5] = ((!flag6) ? num4 : (length + num4)) * num12 / length;
					}
				}
			}
			else
			...
			float[] array = ComputeWorldPositions(pathAttachment, num3, flag4, pathConstraintData.positionMode == PositionMode.Percent, flag3);
			float num13 = array[0];
			float num14 = array[1];
			float num15 = pathConstraintData.offsetRotation;
			bool flag7;
			if (num15 == 0f)
			{
			...
			else
			{
				...
				num15 *= ((!(bone2.a * bone2.d - bone2.b * bone2.c > 0f)) ? (-(float)Math.PI / 180f) : ((float)Math.PI / 180f));
			}
			int num16 = 0;
			int num17 = 3;
			while (num16 < count)
			{
				Bone bone3 = items[num16];
				bone3.worldX += (num13 - bone3.worldX) * num2;
				bone3.worldY += (num14 - bone3.worldY) * num2;
				...
				if (flag5)
				{
					float num22 = exposedList2.Items[num16];
					if (num22 >= 1E-05f)
					{
						float num23 = ((float)Math.Sqrt(num20 * num20 + num21 * num21) / num22 - 1f) * num + 1f;
						bone3.a *= num23;
						bone3.c *= num23;
					}
				}
				...
				if (flag2)
				{
					...
					float c = bone3.c;
					float d = bone3.d;
					float num24 = (flag4 ? array[num17 - 1] : ((!(exposedList.Items[num16 + 1] < 1E-05f)) ? MathUtils.Atan2(num21, num20) : array[num17 + 2]));
					num24 -= MathUtils.Atan2(c, a);
					float num25;
					float num26;
					if (flag7)
					{
						num25 = MathUtils.Cos(num24);
						num26 = MathUtils.Sin(num24);
						float length2 = bone3.data.length;
						num13 += (length2 * (num25 * a - num26 * c) - num20) * num;
						num14 += (length2 * (num26 * a + num25 * c) - num21) * num;
					}
					else
					{
						num24 += num15;
					}
					if (num24 > (float)Math.PI)
					{
						num24 -= (float)Math.PI * 2f;
					}
					else if (num24 < -(float)Math.PI)
					{
						num24 += (float)Math.PI * 2f;
					}
					num24 *= num;
					num25 = MathUtils.Cos(num24);
					num26 = MathUtils.Sin(num24);
					bone3.a = num25 * a - num26 * c;
					bone3.b = num25 * b - num26 * d;
					bone3.c = num26 * a + num25 * c;
					bone3.d = num26 * b + num25 * d;
				}
